...
1. \cite[p.33]{barot2014introduction}交换环 $R$ 上的代数(称为 $R$-代数)，就是一个环 $A$ 和一个包含映射 $\iota : R \to Z(A)$，其中 $Z(A)$ 为 $A$ 的中心，$\iota$ 为一个单的环同态。我们研究较多的是当 $R$ 为一个域 $k$ 时的 $k$-代数。$\iota$ 诱导出以下映射 $r \cdot a = \iota(r)a$ :
\begin{align*}
\cdot : R \times A &\to A\\
(r,a) &\mapsto \iota(r)a
\end{align*}
...
7. 有限生成代数\cite{finitely_generated_algebra}
...
8. 半单模(Semisimple module)\cite{semisimple_module}
...
9. Jacobson radical\cite{jacobson_radical}
...
10. 模的根基(Radical of a module)\cite{radical_of_a_module}
...
11. Socle of a module\cite{socle_of_a_module}
...
12. Quasiregular element\cite{quasiregular_element}
...
13. Essential extension\cite{essential_extension}
