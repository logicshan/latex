...
\]
的形式（并不要求唯一）。也就是说，$\otimes$的像生成了$M \otimes_R N$。 更进一步，如果$f$对每一个元素$x \otimes y$都给出了$Abel$群$G$中的一个值，那么$f$可以扩展为定义在整个$M \otimes N$上的群同态当且仅当$f(x \otimes y)$对$x$和$y$都是$\mathsf{Z}$- 双线性的。
\end{prop}
\begin{proof}
